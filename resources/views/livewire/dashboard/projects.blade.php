<div>
    <input type="text" wire:model.live="search" placeholder="Pesquisar projetos..." class="border-gray-300 rounded p-1 w-1/3">
    <table class="border-separate border w-full overflow-hidden">
        <thead>
            <tr>
                <th class="border">Id</th>
                <th class="border">Titulo</th>
                <th class="border">Criador do projeto</th>
                <th class="border">Candidatos</th>
                <th class="border">Expiração</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="border">{{ $p->candidates->count() }}</td>
                <td class="border">{{ $p->expiration->format('d/m/Y') }}</td>
                <td class="border flex gap-1 w-full">
                    ...
                    <x-modal width="w-full" color="bg-red-400 hover:bg-red-500">
                        ...
                        <div class="flex flex-col gap-4">
                            <div>
                                <h1 class="font-bold text-xl">Tem certeza que deseja excluir este projeto?</h1>
                                <h2>Essa ação é irreversivel</h2>
                            </div>
                            <form action="{{ route('admin.project.destroy',$p->id) }}">
                                @csrf
                                @method('DELETE')
