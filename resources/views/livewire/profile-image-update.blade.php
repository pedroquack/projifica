<div>
    <form wire:submit.prevent="update" method="post" class="flex flex-col gap-5">
        ...
        <label for="image" class="col-span-2 h-24 p-2 text-center bg-white rounded-lg text-black" id="drop-area">
            <input type="file" accept="image/*" id="image" hidden wire:model="image">
            <div id="img-view" class="w-full h-full rounded-lg border-dashed border-2 hover:border-emerald-500 border-emerald-400 bg-emerald-50 hover:bg-emerald-100  cursor-pointer flex items-center justify-center gap-4">
                ...
                <p id="image-txt">Imagem Selecionada</p>
            </div>
        </label>
        @if ($errors->has('image'))
        ...
        @endif
        <button type="submit" class="bg-emerald-400 hover:bg-emerald-500 font-bold text-lg p-2 rounded-lg">
            Atualizar
        </button>
    </form>
    <script>
